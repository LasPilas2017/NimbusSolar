// CRMPanelEtapas.jsx (sólo el render)
export default function CRMPanelEtapas({
  valores = [0, 0, 0, 0, 0],
  porcentajes = [100, 100, 100, 100, 100],
  etiquetas = ["Contacto", "Cita", "Candidato", "Cotización", "Cierre"],
  className = "",
}) {
  const colors = [
    ...
    { border: "#37C796", bar: "#AEEFD8", text: "#0F6B52" }, // verde
  ];

  return (
   <div className={`w-full bg-white rounded-xl shadow-sm border border-amber-200 px-3 pt-2 pb-0 min-h-[70px] md:min-h-[100] h-full ${className}`}>

      {/* Grid fluida: columnas auto-fit con ancho mínimo, centrada y con separación */}
      <div className="
...
function StageCard({ value = 0, percent = 0, label = "", color }) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-stretch justify-between select-none">
      ...
      {/* Etiqueta */}
      <div className="mt-1 text-center text-[13px] font-semibold text-gray-700">
        {label}
      </div>
    </div>
  );
