...
export default function CRMPanelConversion({
  ...
  return (
    <div className={`h-full rounded-2xl border border-amber-200 bg-white/80 p-3 shadow-sm ${className}`}>
      {/* Grid RESPONSIVO:
          - base: 1 col
          - sm:   2 cols
          - lg:   3 cols (siempre dentro del cuadro)
          Usamos fracciones (1fr) y minmax(0,1fr) para evitar overflow.
      */}
      <div className="
          grid gap-2 w-full
          grid-cols-1
        ...
        <div className="min-w-0 w-full flex flex-col">
          ...
          <div className="relative flex flex-col items-center justify-center rounded-b-xl bg-[#AFC7E8] px-3 h-[80px] text-[#2a4a78] text-center w-full">
            <div className="font-semibold tracking-wide text-[12px] flex items-center gap-[4px] mb-1">
              <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden className="translate-y-[1px]">
                ...
                <line x1="8" y1="2" x2="8" y2="6" stroke="#2a4a78" strokeWidth="2" />
                <line x1="16" y1="2" x2="16" y2="6" stroke="#2a4a78" strokeWidth="2" />
              </svg>
...
function Tile({ className = "", children }) {
  return (
    <div className={`w-full rounded-xl p-3 shadow-sm ring-2 ring-amber-200/70 ${className}`}>
      {children}
    </div>
  );
}

/* ------- Barra derecha (alineada + flexible) ------- */
function RightBar({ label, color, value = 0 }) {
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 text-right tabular-nums tracking-tight">{Number(value).toFixed(2)}%</div>
    </div>
  );
}

/* ------- Alternador con fade centrado sin alterar tamaños ------- */
function SwapFade({ left, right, interval = 2000, fadeMs = 500 }) {
  const [showLeft, setShowLeft] = React.useState(true);

  React.useEffect(() => {
    const id = setInterval(() => setShowLeft((v) => !v), interval);
  ...
  return (
    <div className="flex items-center justify-between w-full relative">
      ...
      <div className="flex items-center justify-between w-full relative">
        ...
        {/* Número */}
        <div
          className="absolute inset-0 flex items-center justify-center px-3 transition-opacity"
          style={{
