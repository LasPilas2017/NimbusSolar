...
function DotCompra({ valor, onClick }) {
  ...
  return (
    <button
      ...
      style={{
        width: 12, height: 12, borderRadius: "50%", background: fill,
        border: `1px solid ${THEME.border}`, boxShadow: "inset 0 0 0 2px rgba(255,255,255,.6)"
      }}
...
function Cell({ children, strong, align = "left", rightBorder = true }) {
  return (
    <div
      className={`px-4 py-2.5 text-gray-900 ${strong ? "font-semibold" : ""} ${
...
export default function Diseno({ data = [] }) {
  const COLS =
  ...
  const abrirContactos = (row) => {
    setOpenRow(row);
    const list = Array.isArray(row.contactos) && row.contactos.length > 0 ? row.contactos : [];
    setContactos(list);
  };
  const cerrarContactos = () => { setOpenRow(null); setContactos([]); };
    ...
    <div className="w-full h-auto min-h-screen px-6 pb-10 overflow-y-auto">
      ...
      >
        ...
        <div className={`min-w-[2110px] grid ${COLS}`}>
          {[
            "ID","Nombre Completo","Empresa","Correo Electrónico","Teléfono","Celular","Dirección",
            "Departamento","Municipio","Segmento","Tipo de Instalación","Promedio / kW","Promedio / Q",
            "Fecha de Creación","Días Último Contacto","Estado","Última Fecha de Venta","Compras",
          ].map((h, i, all) => (
            <div
              key={h}
              className="px-4 py-3 font-bold"
              style={{
                background: THEME.headerDark, color: THEME.headerText,
        ...
        <div className="relative min-w-[2110px]" style={{ background: THEME.stripeAlt }}>
          {data.map((r, idx) => {
            ...
            return (
              <div key={r.id} className={`grid ${COLS}`}
                ...
                <Cell>
                  {r.correo && r.correo.includes("@") ? (
                    <a className="text-blue-700 underline" href={`mailto:${r.correo}`}>{r.correo}</a>
                  ) : (r.correo || "-")}
                </Cell>
                ...
                <Cell align="center">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <button type="button" onClick={() => abrirContactos(r)}
                            className="p-1 rounded hover:bg-gray-100" title="Ver registro de contactos">
                ...
                <Cell align="center"><BadgeEstado estado={r.estado} /></Cell>

                <Cell align="center">
                  <div className="flex items-center justify-center gap-2">
                    <span className="tabular-nums">{ultimaVenta}</span>
                    <button type="button" onClick={() => abrirVentas(r)}
                            className="p-1 rounded hover:bg-gray-100" title="Ver historial de ventas">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4"
                           viewBox="0 0 24 24" fill="none" stroke="currentColor"
                           strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <circle cx="11" cy="11" r="8" />
          ...
          <div className={`grid ${COLS}`}>
            <div className="px-4 py-3 font-bold text-white"
                 style={{ background: THEME.header, borderRight: `1px solid ${THEME.border}`, gridColumn: "1 / span 17" }}>
              {totalClientes} Clientes
            </div>
            <div className="px-4 py-3 font-bold text-right text-white" style={{ background: THEME.header }}>
              {fmtQ(totalCompras)}
            </div>
          </div>
        </div>
      ...
      {/* Modal Contactos */}
      <Modal open={!!openRow} onClose={cerrarContactos}
        ...
        <div className="border rounded-xl overflow-hidden">
          <div className="grid grid-cols-[140px_200px_1fr] font-semibold text-white"
          ...
          <div>
            {contactos.map((c, i) => {
              ...
              return (
                <div key={i} className="grid grid-cols-[140px_200px_1fr]"
                  ...
                  <div className="px-3 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                           width="16" height="16" stroke={arrowColor} fill="none"
                           strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
                           title={c.tipo || ""}>
                        <path d={arrowPath} />
                      </svg>
      ...
      {/* Modal Ventas (placeholder) */}
      <Modal open={!!openVentasRow} onClose={cerrarVentas}
        ...
        <div className="rounded-xl border p-6 text-center text-gray-600">
          ...
          <p>Aquí verás el detalle de ventas por fecha, monto y observación. 🚧</p>
        </div>
      </Modal>
    </div>
  );
